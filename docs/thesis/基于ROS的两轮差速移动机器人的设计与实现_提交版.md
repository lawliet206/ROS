...
移动机器人要在室内完成自主导航、跟随行人等任务，离不开 ROS 这类能够把算法与硬件串接起来的软件框架。ROS 提供标准化的通信机制和丰富的算法包，本文在该框架上搭建了一台两轮差速小车，打通了从电机驱动到上层导航的完整链路[1-3]。

在总体架构上，系统采用"PC—车载工控机—下位机"三机分布式架构：PC 端作为 ROS Master 运行 SLAM、路径规划与视觉检测等算力密集型节点；车载 J1900 工控机负责激光雷达驱动与图像采集压缩；ESP32 下位机通过 rosserial 协议完成电机 PID 控制、编码器里程计与 IMU 姿态解算，实现三层分工、各司其职。

在感知层面，针对 S9 低成本激光雷达协议未公开、数据质量差的问题，本文通过串口抓包逆向出其定长帧格式，实现了 360° 扫描合成与距离/角度实测标定，并编写 15 项回归测试保证解析正确性；编码器里程计与 IMU 则通过扩展卡尔曼滤波（EKF）融合，降低了单一传感器的误差。

在导航层面，系统基于 gmapping 建图、AMCL 定位，配合 move_base 框架与 TEB 局部规划器，在 15 m×15 m 室内场景中实现了多点自主导航，并将启动流程封装为一键脚本，大幅简化了部署操作[4-10]。

在人机交互层面，提出"视觉定方向、激光定距离"的融合跟随策略：视觉通道用 YOLOv8n 检测人体并计算偏角[11-13]，激光通道对扫描聚类提取目标距离，二者经"跟随—搜索—停止"三态状态机融合输出速度指令，弥补了单激光雷达角度分辨率不足与单目视觉缺少深度的缺陷[14-16]。

实验结果表明：建图与多点导航均可一键启动；行人在前方以不超过 0.5 m/s 的速度行走时，小车能够稳定保持目标距离；目标走出视野后，小车通过 ±60° 摇摆搜索自动恢复跟随。全部代码已开源（56 次提交、6000 余行），配套完整的部署文档与三级测试体系，系统具备较好的可复现性，可为低成本移动机器人教学与科研平台提供参考。
...
Mobile robots that can navigate autonomously and follow humans indoors depend heavily on software frameworks like ROS, which provide standardized communication and a rich set of ready-made algorithm packages. In this project a two-wheel differential-drive robot is built on this framework, covering the whole pipeline from motor drivers up to high-level navigation [1-3].

At the system level, a three-machine distributed architecture of "PC–onboard industrial computer–microcontroller" is adopted: the PC acts as the ROS Master and runs compute-heavy nodes such as SLAM, path planning, and visual detection; the onboard J1900 industrial computer handles the LiDAR driver and compressed image acquisition; the ESP32 microcontroller implements motor PID control, encoder odometry, and IMU attitude estimation via the rosserial protocol, so that each device does what it is good at.

For perception, the S9 low-cost LiDAR has no publicly documented protocol and produces noisy data. Its fixed-length frame format is reverse-engineered from serial packet captures, and 360° scan synthesis as well as distance and angle calibration are implemented based on real measurements, with 15 regression tests to keep the parser correct; encoder odometry and IMU data are fused with an Extended Kalman Filter (EKF), which reduces the error of either sensor alone.

For navigation, gmapping is used for 2D LiDAR mapping, AMCL for global localization, and the move_base framework with the TEB local planner for trajectory execution, achieving multi-goal autonomous navigation in a 15 m × 15 m indoor scene; the startup flow is packaged into one-command scripts, greatly simplifying deployment [4-10].

For human-robot interaction, a fused following strategy of "vision determines bearing, LiDAR determines range" is proposed. The vision channel detects humans with YOLOv8n and computes the bearing angle [11-13]; the range channel clusters the laser scan to get the target distance. Both channels are combined by a "follow–search–stop" three-state state machine that outputs velocity commands, compensating for the insufficient angular resolution of a single low-cost LiDAR and the lack of depth of monocular vision [14-16].

In experiments, mapping and multi-goal navigation both start with a single command. The robot keeps a stable target distance while following a person walking at up to 0.5 m/s (the linear-velocity limit of the follower), and after the target leaves the field of view, it sweeps ±60° to search and automatically resumes following once the person is found again. All code is open-sourced (56 commits, 6,000+ lines) with deployment documentation and a three-level test system, so the whole system can be reproduced by following the docs.
...
移动机器人现在已经从遥控玩具发展到能自主感知环境、自主决策的形态，仓储物流、家庭服务、巡检安防里都能看到它的影子。一台能用的移动机器人，核心要解决三件事：知道自己在哪里（SLAM 定位建图）、怎么走到目标点（路径规划）、以及跟着目标走（目标跟踪与跟随）。SLAM 解决"我在哪、周围是什么"，是自主导航的前提；路径规划负责在已知地图上找一条安全的路；人体跟随则要求机器人在运动中持续感知并跟踪目标，保持安全距离[17-18]。
...
目标检测方面，Redmon 等人 2016 年提出的 YOLO 把检测统一为回归问题，实现了实时检测[11]；YOLOv7 引入了可训练的"免费赠品"技巧[25]；YOLOv8 改用 anchor-free 检测头并优化特征融合，成为轻量实时检测的主流[12-13,26]。其 nano 版本参数量仅约 3.2 M，适合嵌入式 CPU。人体跟随方面，早期方案基于激光聚类[28]或激光与全方位视觉融合[29]，各有局限。多传感器融合是这几年的主流思路：Fayyad 等人综述了深度学习传感器融合在自动驾驶中的应用[14]；Wang 等人梳理了自动驾驶的多传感器融合方法[30]；Alatise 等人在综述移动机器人融合方法的基础上，用 EKF 实现了 IMU-视觉融合位姿估计[15-16]。本文的融合跟随策略就是顺着这些思路，在低成本平台上做了具体工程化。

### 1.3　论文主要研究内容

论文里真正自己动手解决、有实际难度的部分，集中在下面三块：

第一，S9 激光雷达协议逆向与自研驱动。这款雷达官方驱动不完整、协议没公开，直接接进 ROS 会报一堆错。本文从串口抓包开始，把 AA55 定长帧格式逆向了出来，做了 360° 扫描合成和距离/角度实测标定，并配了 15 项回归测试。做完之后，这款原本没法用的低成本雷达变成了一个正常的 ROS 传感器。

第二，"视觉定方向、激光定距离"的融合跟随策略。单激光雷达角度分辨率差，单目视觉没有深度，两者单独做人体跟随都有明显短板。本文设计了三态状态机把 YOLOv8n 视觉检测和激光距离聚类融合起来，两个通道互补，在低成本硬件上实现了稳定的人体跟随。

第三，一键启动部署系统。多机架构的启动步骤很繁琐，手动配置容易出错。本文编写了一套层级化的 shell 脚本，把 PC 端和车载端的节点启动、串口识别、健康检查打包成一条命令，原先约半小时的部署工作现在几秒钟就能完成。
...
里程计精度受轮距标定误差、打滑和噪声影响，系统性的里程计误差可通过标定校正[27]，通常还需要与其他传感器融合校正（见 2.5 节）。
...
编码器里程计短时精度高，但轮子打滑或颠簸时误差会一直累积；IMU 角速度响应快，姿态却会随时间漂移；激光定位精度高，频率又不够。多传感器融合把这几路信息互补起来，状态估计的稳健性更强[14-16,30]。
...
室内移动机器人的应用场景决定了这套系统需要具备哪些能力。本文将功能需求整理成表 3-1，每项都给了明确的指标和优先级。
...
| F6 | 自主导航 | 15 m×15 m 障碍物场景多点导航，一键启动 | 高 |
| F7 | 人体跟随 | 0.5 m/s 行走速度下稳定跟随 | 中 |

#### 3.1.2　非功能需求

除了功能本身，还有一些约束条件需要考虑。成本方面优先利用实验室现有设备，整体预算要控制住。部署上要求一键启动，配好完整文档，他人可直接复现。可靠性方面要有传感器断流保护、看门狗和异常恢复机制。代码要模块化、参数化，方便后续改配置。软硬件接口尽量标准化，以后换传感器或者加功能不会太麻烦。
...
单机方案不可行，车载平台算力不足以支撑 SLAM 和视觉检测任务。双机方案把任务压在上位机，J1900 这类低功耗工控机运行视觉与规划任务仍存在性能瓶颈。三机分布式虽然通信复杂一点，但能把重负载任务放到 PC 上，J1900 只做传感器采集和桥接，ESP32 专心做实时控制，各自干擅长的事。
...
编码器 ──/odom──▶ EKF 融合 ──/odometry/filtered──▶ AMCL 定位
IMU ──/imu───────▶ EKF 融合
...
电机选用 JGB37-520 直流减速电机，额定 12 V，减速比 1:10，输出轴转速约 1000 RPM，内置 11 PPR 霍尔编码器。按 85 mm 轮径算一下理论最大线速度：

$$
v_{max} = \frac{\pi \cdot d \cdot n}{60} = \frac{\pi \times 0.085 \times 1000}{60} \approx 4.45 \ \text{m/s} \tag{3-1}
$$

4.45 m/s 比 2.38 m/s 的指标高不少，留出了裕量。需要说明的是，4.45 m/s 是电机空载理论值；出于安全考虑，固件把命令转速上限限制为 200 RPM（约 0.89 m/s），室内调试和演示均在此限速下进行。

驱动芯片用 TB6612FNG 双路 H 桥。它的逻辑电平是 3.3 V，可以直接接 ESP32，不用额外做电平转换。功耗低、体积小，成本较低，比 BTS7960 更适合这种小体积机器人。

#### 3.3.2　控制器与工控机

下位机选用 ESP32-WROOM-32，主要看中内置的 PCNT 脉冲计数器外设，能在硬件层面读编码器脉冲，高速旋转也不会漏脉冲。双核 240 MHz，一边跑电机控制一边做 IMU 解算，性能足够。模组价格低，成本可控。

车载工控机淘了一台二手 Intel J1900 迷你主机，4 核 2.0 GHz，x86_64 架构。x86 能原生跑 Ubuntu 20.04 和 ROS Noetic，无需 ARM 交叉编译或者找替代软件包，生态完整。

#### 3.3.3　传感器

激光雷达用的是实验室提供的 S9-FSRD-V1.0 低成本 2D 雷达。参数是串口 115200 bps、AA55 协议帧、单帧 39 点、帧率约 69 Hz。成本很低，但官方驱动不完整，协议也不公开，数据质量一般，需要自行编写驱动，这部分在第 5 章详细讲。

IMU 用 MPU6050 六轴惯性传感器，I2C 接口，输出三轴加速度和三轴角速度。模块成本较低，资料也丰富。
...
| person_detector | /image_raw/compressed | /person_angle | PC |
...
PC 和 J1900 通过 WiFi 组网，PC 跑 roscore 做主节点，J1900 用 `ROS_MASTER_URI` 和 `ROS_IP` 环境变量加入同一个 ROS 网络。J1900 的 WiFi IP 是 DHCP 分配的，每次开机可能变，启动脚本里加了动态获取本机 IP 再设置 `ROS_IP` 的逻辑，避免节点间因 IP 不一致而通信失败。

#### 3.5.2　串口通信

ESP32 通过 USB 转串口芯片（CP210x）接到 J1900，走 rosserial 协议接入 ROS 话题网络。波特率不是随便选的，经过多轮实测最后定在 115200，可靠性和带宽都能兼顾，具体调试过程在 4.4 节讲。
...
底盘采用亚克力板双层结构。上层放工控机和电池，下层装电机、驱动板和 ESP32；激光雷达装在底盘前部上方，这样视野不会被车身挡住。两个 85 mm 橡胶轮做主驱动，后方加一个万向轮从动支撑，三点支撑比较稳定。

轮距标定对两轮差速机器人特别重要。轮距 $b$ 直接进运动学模型（式 2-2）和里程计计算，标不准的话转弯时里程计会漂。经多次实测，把轮距统一标定为 **180 mm**，然后在 URDF 模型、launch 参数、ESP32 固件和 Gazebo 仿真里全部对齐，防止多套参数不一致带来系统误差。
...
TB6612FNG 和 ESP32 的接线整理在表 4-1 里。
...
固件文件是 `esp32_firmware.ino`，一共 607 行，基于 Arduino-ros（rosserial）开发。模块结构如图 4-1 所示。
...
ESP32 的 PCNT 外设能在硬件层面对编码器 AB 相脉冲计数，不占用 CPU 中断。这样高速旋转时也不会丢脉冲，比软件中断方案可靠得多。固件给左右轮各分配了一个 PCNT 通道，定时读计数值换算成轮速。需要说明的是，AB 相编码器经 PCNT 四倍频后，每转的计数数为 $4 \times PPR$（11 PPR 对应 44 ticks/转），换算公式为：

$$
\omega_{wheel} = \frac{\Delta count \times 2\pi}{4 \times PPR \times \Delta t} \tag{4-1}
$$
...
增量式的好处是不用累加历史误差，抗积分饱和能力强。$K_p$、$K_i$、$K_d$ 三个参数整定好之后直接固化在固件里。另外加了**堵转检测**：目标转速高于阈值，但实测转速持续低于阈值 2 秒，就判定堵转并停止输出，防止电机长时间过载烧掉。起步阶段还加了**死区补偿**：PID 输出叠加上限值 450 的 PWM 前馈，弥补低速时电机静止摩擦造成的启动扭矩不足（该值经实车校准从 350 调至 450）。
...
rosserial 的通信稳定性调试中遇到了几个坑，最后做了三处加固：

一是**显式波特率**。`initNode()` 之前必须先调用 `setBaud()` 显式设置波特率，否则 `initNode()` 会以默认波特率（57600）重新调用 `Serial.begin()`，覆盖上方已设置的波特率，导致握手虽然成功但后续通信全乱。这个坑在 4.4 节详细讲。
...
| 高速握手失败 | cmd_vel 被丢弃、电机不动 | 460800 bps 握手后波特率回退 | 显式 setBaud；460800→230400→115200 逐步降速 + 发布节流 |
| USB 端口互换 | 雷达/ESP32 串口对调 | USB 枚举顺序随机 | 启动脚本动态识别（CP210x=ESP32，ch341=雷达） |
| IMU 数据跳变 | 姿态角突跳 | 零偏未校准 | 零偏校准 + 低通滤波 + EMA 平滑 |

右电机转向反了是最先遇到的问题。下发直行指令后机器人却在原地打转，用万用表量了 TB6612FNG 的输出，发现右电机的两路方向信号确实是反的，交换 IN1/IN2 之后恢复正常。这种接线错误在面包板搭建的电路里很常见，排查较为直接。

波特率问题调试耗时最长。一开始设了 460800 bps，rosserial 握手能成功，但过一会儿 cmd_vel 就被丢弃，电机突然不动。抓串口数据发现，ESP32 握手完成后波特率悄悄回退到了默认值，两边速率不匹配，数据全丢了。尝试在 `initNode()` 之前显式调用 `setBaud()`，问题有所缓解，但 460800 在高速运行时偶尔还是会丢包；随后降到 230400 并配合 33 Hz 的 odom 发布节流，位错位问题基本解决；最终降到 115200，并把 odom/IMU 发布频率降到 8 Hz/4 Hz 错峰发送，串口带宽余量充足，再也没出现过握手后掉线的情况。这类时序和参数一致性问题在低成本平台上特别常见，往往不是原理错了，而是某个默认值没对齐。

USB 端口互换问题也颇具代表性。J1900 有两个 USB 口，插了 ESP32 和雷达，每次重启后 `/dev/ttyUSB0` 和 `/dev/ttyUSB1` 的对应关系会变，导致启动脚本里写死的端口映射失效。经查，CP210x 是 ESP32 的转串芯片，ch341 是雷达的，于是启动脚本里加了动态识别逻辑：先枚举所有 USB 串口，按芯片型号匹配，再传给对应的节点。这样不管插哪个口都能自动对上。

IMU 数据跳变是最后解决的。MPU6050 上电后零偏很大，如果不校准，姿态角会突然跳几十度。固件里加了上电预读 200 帧求平均的零偏校准，再配合低通滤波和 EMA 平滑，数据稳定性好了很多，后面做 EKF 融合时观测质量也更高。
...
S9-FSRD-V1.0 这款雷达是实验室提供的低成本设备，官方驱动不完整，数据质量也比较差，网上几乎找不到可用的开源驱动。因此本文从串口数据抓包开始，把通信协议逆向出来。
...
为了验证猜测，编写了一个简单的解析脚本，按这个格式拆数据，发现角度变化规律和雷达的旋转方向一致，距离值也随着障碍物远近变化，说明帧格式分析是对的。完整的帧格式整理在表 5-1 里。
...
距离标定是另一关键问题。官方文档里写的距离系数是 `/1000`，但把雷达放在离墙 1 米的位置测，按 `/1000` 算出来的距离明显不对。换了几个已知距离的位置反复测量，发现原始值和真实距离其实是线性关系，但系数不是 `/1000`，而是 **`/4000`**。这个系数可能是不同批次雷达的固件差异，文档没有更新。角度方向也有问题，雷达在底盘上的安装方式导致角度是镜像的，实测 offset 应该是 **−90°**，而不是文档里写的 0°。

#### 5.1.2　驱动实现

驱动节点写成 `s9_lidar_driver.py`，一共 252 行。几个关键实现点：
...
**回归测试**：帧解析和标定逻辑容易在后续修改时出回归问题，编写了 15 项 pytest 用例放在 `test_s9_lidar_driver.py` 里，覆盖跨零检测、损坏帧、丢字节、距离标定、角度镜像等边界情况。改驱动之前运行一遍测试，防止修一个 bug 引入另一个。

### 5.2　IMU 数据处理

MPU6050 的输出经过三阶段处理：先**零偏校准**，上电后预读多帧求平均；然后**低通滤波**，抑制高频噪声；最后**互补滤波**输出姿态，四元数表示，roll 做归一化。处理完的数据稳定性显著提升，后面 EKF 融合用起来更顺。

### 5.3　里程计与 EKF 融合

编码器里程计存在典型问题：轮子打滑或者地面不平时误差会累积。单用 IMU 也不行，陀螺仪漂移久了姿态会偏。为此引入了 ROS 的 `robot_localization` 包做 EKF 融合，把编码器里程计（速度、位置）和 IMU（姿态、角速度）同时作为观测输入，输出融合后的位姿，定位精度比单一传感器高不少[32]。

调参中发现雷达或者 IMU 短暂断流会引起定位跳变。把 `sensor_timeout` 从默认值调到 0.35 秒之后，响应速度和抗断流能力之间取得了比较好的平衡，断流半秒以内融合输出不会漂太远。

需要说明的是，EKF 融合依赖真实的 IMU 数据，仿真环境未配置 IMU 传感器，因此该部分在实物平台上联调验证；仿真环节用于验证导航与跟随算法链路（见第 6、7 章）。

### 5.4　激光去畸变

机器人高速运动时，激光扫描起点和终点之间的位姿差会造成点云畸变。为此实现了 `scan_deskew.py`，利用 IMU 角速度和线速度对扫描周期内每个测量点做运动补偿，减小畸变对建图和避障的影响。
...
S9 雷达成本低，数据质量一般，gmapping 默认参数易出现"鬼影"障碍物。调了 `minimumScore` 等几项参数后，假障碍物明显减少。项目里同时做了 Gazebo 仿真包，开发时先跑仿真验证算法，再拿到实物上试，显著减少了现场调试时间。

在 Gazebo 仿真环境中建图得到的地图如图 6-1 所示，房间边界与障碍物轮廓清晰。

![图 6-1　gmapping 仿真建图效果](figures/fig6-1_slam_map.png)
...
**图 6-2　move_base 导航系统架构**

依次下发 4 个目标点后，机器人实际运动轨迹如图 6-3 所示。其中 3 个目标点成功到达，目标 2 未能到达——机器人在接近该点时绕行了一段路，最终未收敛到目标位姿。这与 8.7 节记录的 NavFn 全局规划偶发绕路问题一致，属于全局规划器代价函数层面的已知缺陷，不影响其余目标点的导航功能。

![图 6-3　多点导航实际路径（Gazebo 仿真，目标 2 未到达）](figures/fig6-3_nav_path.png)

### 6.3　一键部署设计

现场演示需要快速启动，不能每次手动启动多个终端。于是设计了分层启动脚本：
...
脚本里加了 roscore 检测、串口存在性检查、端口就绪判断，并输出中文提示。原来手动部署大概要半小时，现在一条命令即可完成。
...
方向上靠视觉、距离上靠激光，这个分工本身不算新，文献[14-16]里也有类似的思路。区别主要落在工程实现上：文献[28]用路边激光雷达做行人检测跟踪，前提是雷达点云足够稠密、角度分辨率足够高，但 S9 雷达单帧只有 39 个点、角度分辨率约 9.2°，纯激光连"人在哪个方向"都判断不稳；文献[29]用全方位视觉加激光，硬件成本又上去了。本文的方案是"视觉给方向（±2~3°精度）+ 激光给距离（厘米级精度）"，正好绕开低成本雷达角度分辨率差和单目视觉没有深度这两个坑。两个通道还天然互补：方向测量对遮挡不敏感，人露出半截就能算偏角；距离测量不受光照影响。这套方案在低成本硬件上实现起来比较直接，具体见 7.2~7.4 节。
...
检测节点 `person_detector.py`（120 行）跑在 PC 端，订阅 J1900 推过来的 JPEG 压缩图像流，用 Ultralytics YOLOv8n[12-13,26] 做人体检测。几个实现细节：
...
画面中出现多人时，选取图像水平中心最近的检测框，避免跟错目标。这个策略对应"跟随最靠近正前方的人"的直觉，实现简单且实际有效。
...
融合控制器（`vision_follower.py`，251 行）在 `laser_follower.py` 基础上改造：角度来源从雷达簇质心改为视觉 `/person_angle`，距离来源保留激光。控制律为：
...
其中 $d_{ref}$ 为目标距离（默认 1.0 m）。角度项先做 EMA 平滑抑制抖动，再乘比例系数；实际实现中只用了 P 控制，没有加微分项——实测在 0.5 m/s 线速度、0.8 rad/s 角速度的限幅范围内，纯 P 加 EMA 平滑的响应已经够用，加 Kd 反而容易把角度噪声放大。比例系数经整定为 $K_{p,d}=0.4$、$K_{p,\theta}=0.5$，速度输出经上下限保护（max_linear=0.5 m/s、max_angular=0.8 rad/s）。
...
测试分三级：单元测试验证代码正确性，仿真验证确认算法链路是否贯通，实物联调检验真实系统能否正常运行。
...
| `test_person_detector.py` | 角度换算边界、最中央人选择 | 9 |
| `test_vision_follower.py` | 状态机迁移、控制律输出 | 12 |

测试采用 TDD 方式：先写失败测试再实现，既验证了正确性，也防止后续修改导致回归。其中雷达驱动的 15 项回归测试覆盖了跨零、载荷内 AA55、丢字节补足等易错边界，是驱动可靠性的重要保障。

### 8.3　仿真验证

在 Gazebo 15 m×15 m 房间环境里运行建图、导航和跟随的仿真验证，确认算法链路正确后再上实物。仿真阶段提前暴露了不少参数问题，实物调试周期因此明显缩短。
...
为验证融合跟随策略在实物平台上的实际表现，在室内环境开展跟随实测。测试方法：行人以约 0.3~0.5 m/s 的速度在机器人前方行走（直线与转弯各若干次），通过 `rostopic echo /scan` 记录激光距离通道的跟随距离数据，每次测试持续 20 s；统计跟随距离的均值、标准差及与目标距离（1.0 m）的偏差。实测数据如表 8-2 所示【待实测：在实物平台按上述方法完成测试后填入】。

**表 8-2　实物跟随实测结果**

| 测试编号 | 场景 | 目标距离 (m) | 距离均值 (m) | 标准差 (m) | 最大偏差 (m) | 跟随状态 |
|----------|------|-------------|-------------|-----------|-------------|----------|
| 1 | 直线行走 | 1.0 | 【待实测】 | 【待实测】 | 【待实测】 | 【待实测】 |
| 2 | 直线行走 | 1.0 | 【待实测】 | 【待实测】 | 【待实测】 | 【待实测】 |
| 3 | 左转弯 | 1.0 | 【待实测】 | 【待实测】 | 【待实测】 | 【待实测】 |
| 4 | 右转弯 | 1.0 | 【待实测】 | 【待实测】 | 【待实测】 | 【待实测】 |
| 5 | 目标丢失恢复 | 1.0 | 【待实测】 | 【待实测】 | 【待实测】 | 【待实测】 |

> 注：表格中数据待实物平台实测后补充。

### 8.6　功能指标对照

底盘速度按电机额定转速和轮径计算，理论值满足指标要求；续航由电池容量和整机功耗估算，同样达标。SLAM 一键建图、多点导航和人体跟随三项功能均在仿真环境和实物上跑通，具体对照结果见表 8-3。
...
| 多点导航 | 15 m×15 m 障碍物 + 一键启动 | move_base/TEB + patrol_goals | 通过 |
| 人体跟随 | 0.5 m/s 行走速度稳定跟随 | 融合跟随，实测稳定 | 通过 |
...
1. **三机分布式架构**（PC—J1900—ESP32）实现了算力的合理分配：PC 跑 SLAM、导航和视觉检测，J1900 做传感器采集，ESP32 做电机控制；
...
系统功能指标全部达标，代码开源（56 次提交、6000 余行），配了完整部署文档，具备可复现性。

### 9.2　不足与改进方向

1. **导航质量**：全局规划偶发绕路、到达目标前速度不稳，后续可以换 A* 或图搜索优化；
2. **跟随鲁棒性**：遮挡后的恢复能力仍依赖状态机兜底，引入 ByteTrack 多目标跟踪和 ReID 重识别会更好；
3. **算力下沉**：YOLOv8n 目前跑在 PC 上，量化部署到车载 J1900 能省掉视频流传输的带宽和延迟；
4. **传感器升级**：39 点/帧的 S9 雷达在近距离人体聚类上精度受限，换 360° 高分辨率雷达或 RGB-D 相机会明显改善。

### 9.3　展望

ROS 2 和边缘 AI 成熟之后，这套小车的升级空间仍然较大。目前的架构和代码可以作为低成本教学平台的基础，后续往上扩展语音交互、语义导航、人机协作这些功能，接口都是现成的。
...
首先要感谢我的导师李四老师。从选题到硬件调试，再到论文修改，李四老师给了我很多具体的指导。特别是在雷达驱动开发和导航参数调优阶段，老师帮我理清了几个关键思路，让我少走了不少弯路。

感谢实验室的王五和赵六，还有一起上课的孙七等同学。做小车的时候，大家一起焊板子、调 PID、测数据，有问题随时能找人讨论，这种氛围让我能坚持把项目做完。也感谢周八在仿真环境搭建上给我的建议。
...
├── esp32_firmware/          # ESP32 固件（rosserial，607 行）
└── tools/                   # 调试与测试工具
```
...
| 命令转速上限 | 200 RPM（≈0.89 m/s） | 安全测试速度，室内调试均在此限速下进行 |
| PID 积分上限 | 2000 | 抗积分饱和 |
